...
import pandas as pd
from before_adult import convert
...
def main(
    ...
    converted_before_df = convert(before_df)  # 各質問項目を数値に変換する
    ...
    qa_adult_results_df.to_csv("./qa_adult_results.csv", index=False)
    #
    # # 児童思春期のアンケートの処理
    # # TODO: 横方向に連結→before_child_parent_columnsの順番で取り出す→convertを作って投げる
    # igaku_child_before_df = pd.read_csv(igaku_child_before_file_path)
    # igaku_parent_before_df = pd.read_csv(igaku_parent_before_file_path, skiprows=1) # 1行目には各項目をまとめたヘッダーが書かれているのでスキップする
    # igaku_parent_before_df.rename(columns={'No': 'ID'}, inplace=True) # No列をID列に変更する
    # igaku_child_parent_before_df = pd.merge(igaku_child_before_df, igaku_parent_before_df, on='ID', how='inner') # ID列をキーにして横方向に結合する
    # extracted_igaku_child_parent_before_df = extract_columns(igaku_child_parent_before_df, before_child_parent_columns)
    # # extracted_igaku_child_parent_before_df.to_csv("igaku_child_parent_before_df.csv", index=False)
    # # converted_child_parent_before_df =
    # extracted_igaku_child_after_df = extracted_igaku_after_df[~extracted_igaku_after_df['ID'].isin(igaku_before_df['ID'])]
    # # extracted_igaku_child_after_df.to_csv("igaku_child_after_df.csv", index=False)
    # # converted_child_after_dfと横方向で連結する
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--riko_before_file_path",
        type=str,
        default="../data/qa/riko_before.csv",
        help="理工学部面談の事前アンケートのファイルパス",
    )
    parser.add_argument(
        "--igaku_before_file_path",
        type=str,
        default="../data/qa/igaku_before.csv",
        help="医学部面談（成人）の事前アンケートのファイルパス",
    )
    parser.add_argument(
        "--igaku_child_before_file_path",
        type=str,
        default="../data/qa/igaku_child_before.csv",
        help="医学部面談（児童思春期）の事前アンケートのファイルパス",
    )
    parser.add_argument(
        "--igaku_parent_before_file_path",
        type=str,
        default="../data/qa/igaku_parent_before.csv",
        help="医学部面談（保護者）の事前アンケートのファイルパス",
    )
    parser.add_argument(
        "--riko_after_file_path",
        type=str,
        default="../data/qa/riko_after.csv",
        help="理工学部面談の事後アンケートのファイルパス",
    )
    parser.add_argument(
        "--igaku_after_file_path",
        type=str,
        default="../data/qa/igaku_after.csv",

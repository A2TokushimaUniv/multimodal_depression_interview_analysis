...
for ((i = 0; i < $igaku_file_count; i++)); do
  ...
  python3 main.py --input_video="${igaku_mp4_file_array[$i]}" --input_audio="${igaku_m4a_file_array[$i]}" --output_dir=$output_dir --faculty=igaku --dir_num=$dir_num
done
...
for ((i = 0; i < $riko_file_count; i++)); do
  ...
  # 後ろから2番目のディレクトリを取得
  dir_num=${path_parts[$((path_length - 1))]}
  python3 main.py --input_video="${riko_mp4_file_array[$i]}" --input_audio="${riko_m4a_file_array[$i]}" --output_dir=$output_dir --faculty=riko --dir_num=$dir_num
done
